...
export default async function ColorsPage() {
    ...
    return (
        <div className="container mx-auto max-w-4xl px-4 py-12 pt-[calc(var(--header-total-height)+3rem)]">
            <h1 className="text-4xl font-bold mb-8">Colors</h1>
                ...
                {colors.map((color) => (
                    ...
                    >
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-500">{color.productCount} product{color.productCount !== 1 ? 's' : ''}</p>
                        </div>
                    </Link>
                ))}
            </div>
            {colors.length === 0 && (
                <p className="text-gray-500 text-center py-12">No colors found.</p>
            )}
        </div>
    );
}
