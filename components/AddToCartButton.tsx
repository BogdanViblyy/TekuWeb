    ...
    <form action={addToCartAction} className="space-y-4">
      {product.availableColors.length > 0 && (
          <div className="space-y-2">
              <label htmlFor="color" className="text-sm font-medium">{tProduct('selectColor')}</label>
      ...
      {product.availableSizes.length > 0 && (
           <div className="space-y-2">
              <label htmlFor="size" className="text-sm font-medium">{tProduct('selectSize')}</label>
      ...
      <SubmitButton text={tProduct('addToCart')} loadingText={tProduct('adding')} />
    </form>
  );
}